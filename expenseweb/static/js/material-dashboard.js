...
function darkMode(el) {
  ...
  const r_nav_i = document.querySelectorAll('div > .text-dark');
  ...
  if (!el.getAttribute("checked")) {
    ...
    for (var i=0; i< r_nav_i.length; i++) {
      if(r_nav_i[i].classList.contains('text-dark')){
      r_nav_i[i].classList.remove('text-dark');
      r_nav_i[i].classList.add('text-white');
      }
    }
    ...
    for(var i = 0; i < right_nav.length; i++){
      if(right_nav[i].classList.contains('dark')){
        right_nav[i].classList.remove('dark');
        right_nav[i].classList.add('white');
      }
    }
