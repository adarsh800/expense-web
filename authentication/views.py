...
class Validate_Username_View(View):
...
class Validate_Email_View(View):
    def post(self, request):
        data = json.loads(request.body)
        email = data['email']
        ...
        if User.objects.filter(email=email).exists():
            return JsonResponse({'email_error': 'sorry email in already in use, choose another one'}, status=409)
    ...
    def post(self, request):
        ...
        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(email=email).exists():
                if len(password) < 6:
                    messages.error(request,'Error: Password too short')
    ...
    def get(self, request, uidb64, token):
        try:
            ...
            if not account_activation_token.check_token(user,token):
                return redirect('sign-in'+'?message='+'User alredy activated')
